...
在上一课时学习了计算机的组成原理后，本课时将继续深入学习程序执行部分，进一步讨论程序在冯诺依曼模型上如何执行。
...
了解了基本的程序执行过程后，接下来看看如果用冯诺依曼模型执行 a=11+15 是一个怎样的过程。

程序员写的程序 a=11+15 是字符串，CPU 不能执行字符串，只能执行指令。所以这里需要用到一种特殊的程序——编译器。编译器的核心能力是翻译，它把一种程序翻译成另一种程序语言。

这里需要编译器将程序员写的程序翻译成 CPU 认识的指令(指令我们认为是一种低级语言，平时书写的是高级语言)。

下面详细阐述 a=11+15 的执行过程:
...
* 为什么 0x200 中代表加载数据到寄存器的指令是 0x8c000100，这个会在下面详细讨论
...
在上面的例子中，load 指令将内存中的数据导入寄存器，写成了 16 进制: 0x8c000100，拆分成二进制就是:
...
这里把操作码、寄存器的编号、要读取的地址合并到了一个 32 位的指令中。

对于一条求加法运算的 add 指令，16 进制表示是 0x08048000，换算成二进制就是:
...
* 首先 CPU 通过 PC 指针读取对应内存地址的指令，这个步骤叫作 Fetch，就是获取的意思
...
上面 4 个步骤为 CPU 的指令周期。CPU 的工作就是一个周期接着一个周期，周而复始。

---

### 指令的类型

通过上面的例子可以发现不同类型(不同 OpCode)的指令、参数个数、每个参数的位宽，都不一样。而参数可以是以下这三种类型:
...
* 关于寻址模式和所有的指令，如果你不是嵌入式开发人员，就不需要记忆，理解即可
...
之前提到， CPU 是用石英晶体产生的脉冲转化为时钟信号驱动的，每一次时钟信号高低电平的转换就是一个周期，称为时钟周期。CPU 的主频，说的就是时钟信号的频率。比如一个 1GHz 的 CPU，说的是时钟信号的频率是 1G。

---

## 总结

接下来做一个总结。这节课深入讨论了指令和指令的分类。接下来看一看在 02 课时中留下的问题: 64 位和 32 位比较有哪些优势？
...
最后再给出一道思考题: CPU 中有没有求对数的指令？如果没有那么程序如何去计算？
