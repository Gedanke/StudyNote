...
关于某个函数求解极值的问题，从中学就开始接触了。那时候的求解方法是，令目标函数的一阶导数为零，并求解方程的解，这种方法称作求导法。

例题 1: 假设你是某电商公司营销系统的工程师。你们某个商品的购买概率和补贴额的关系为，$p(x) = 0.05 x + 0.2$。该商品原价 m 为 16 元，成本价 c 为 8 元，求利润最大的补贴额应该是多少？

商品的利润函数为 $r(x) = p(x)\times(m - x - c) = (0.05x + 0.2)×(16 - x - 8) = -0.05x2 + 0.2x + 1.6$，利用高中的数学求导法，令 $r(x)$
的导函数为零并解方程，则有:
...
我们把这个过程用数学语言来重新描述。先来定义一下函数的梯度，对于函数 $f(x,y)$，常用 $\nabla f(x,y)$ 来表示函数的梯度。其中 x、y
...
例题 2: 按照之前得到的商品利润函数 r 和补贴金额 x 的关系为 $r(x) = p(x)×(m - x - c) = (2/(1+e-x) - 1)×(16 - x - 8)$。下面利用梯度下降法，求解让利润最大的补贴额
$x^*$。
