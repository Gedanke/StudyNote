...
到这里发现，对折 10 次后，厚度也不过才刚刚达到 1 m。也许你会不仅感慨，以这样的速度，何时才能到达月球啊。
...
指数运算，即幂运算，写作 $a^n$，其中 a 为底数，n 为指数:
...
例如，如果把每一位的密码，由先前的数字调整为数字或区分大小写的字母，则意味着密码的搜索空间由 $S = 10^n$，提高到 $S = 62^n$。
