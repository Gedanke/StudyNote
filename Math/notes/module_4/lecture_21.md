...
最后利用 AI 框架的第三个公式 $w^*= arg min L(w)$，来求解神经网络。在神经网络中，w 系数就是所有的 $w_{ijk}$。
...
其中，对于某个给定的数据集而言，$x_i$ 和 $\bar{y}_i$ 都是已知的。也就是说，要求解出让上面损失函数 $L(w)$ 取得极小值的 $w_{ijk}$ 的值，因此可以考虑用先前学的随机梯度下降法来进行求解。
...
这一讲学习了神经网络和深度学习。在当前的 AI 时代下，深度学习模型在效果方面打败了传统的浅层模型。而深度学习的基本原理都主要来自神经网络，神经网络结构可被"画出"，输入、输出之间可以用函数表达，这些特点都是支持它深度化的前提。
...
最后留一个练习题: 假设有一个 4 层的神经网络，第一层是输入 xi，最后一层是输出 y。4 层的结点数分别是 3、2、2、1。试着去求解一下损失函数关于每个链接权重的梯度吧。
